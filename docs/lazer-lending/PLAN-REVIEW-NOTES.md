...
Driven by:
- `tmp/review-notes/2026-05-01-codex-feasibility-audit.md` (4-lens audit: Depth, Breadth, Adversary, Gaps)
- `tmp/research/2026-05-01-feasibility-validation.md` (external validation: 80+ sources)

### High-confidence findings (multi-lens agreement) and resolutions

| # | Finding | Severity | Resolution location |
|---|---------|----------|---------------------|
| 1 | Atomic dispatcher leaks slots on Smartlead 5xx | Critical bug | PLAN.md §claimSendSlot (Fix 1) |
| 2 | Watchdog hard-complaint rule is dead code via else-if chain | Critical bug | PLAN.md §runMailboxWatchdog (Fix 3) |
| 3 | FUB email_normalized over-merges Outlook/Yahoo (Gmail-dot rule applied globally) | Critical bug | PLAN.md §normalizeEmail (Fix 9) |
| 4 | Webhook handler does too much work inline; Smartlead retries cause double-process | Critical bug | PLAN.md §Webhook Idempotency + D23 (Fix 7) |
| 5 | HMAC unsubscribe token has no rotation story; uses non-timing-safe compare | Security | PLAN.md §verifyUnsubToken (Fix 8) |
| 6 | Wilson watchdog dormant at v1 volume | Documentation gap | PLAN.md §Pacing & Concurrency honesty note (Fix 4) |
| 7 | Stop-on-reply exception hammers mis-classified positives | Logic | PLAN.md §applyClassification + D22 (Fix 6) |
| 8 | Classifier missing regex backstop for "stop / remove / unsubscribe" | Critical compliance | PLAN.md §classifyReply + D21 (Fix 5) |
| 9 | 3 of 7 PRD ship criteria silently replaced without client signoff | Contractual | New doc: docs/lazer-lending/PRD-AMENDMENT.md |
| 10 | NMLS / state-licensing footer punted to "Lazer compliance/legal supplies" with no default | Project-blocking compliance | New doc: docs/lazer-lending/COMPLIANCE.md + Phase 1 Task 1.0c |
| 11 | No source-of-list / consent tracking on Lead model | Compliance | PLAN.md §Data Models > Lead (add fields) + COMPLIANCE.md |
| 12 | No subpoena-ready audit artifact | Compliance | COMPLIANCE.md §9 + PLAN.md §Final Validation Checklist |
| 13 | Phase 1 timeline fictional (8–12 weeks reality vs implicit 2–4 weeks) | Schedule | PLAN.md §Schedule Reality (new section in Confidence Score) |
| 14 | Mailforge "5–10% deplatform" undefended; Smartlead-Workspace pairing is Google's named trigger | Risk-model error | BRIEF D8 + D9; PLAN.md state machine adds `standby` |
| 15 | No auth / RBAC strategy for production | Blocks v1 | PLAN.md Phase 1 Task 1.0 |
| 16 | No CCPA right-to-delete (GLBA exemption is data-level, not entity-level) | Compliance | PLAN.md Phase 1 Task 1.0b + COMPLIANCE.md §7 |
| 17 | No suppression-list seed import from existing FUB/legacy | Blocks v1 | PLAN.md Phase 0 Task 0.10 |
| 18 | No SLA / termination clause / data-export | Commercial | New doc: docs/lazer-lending/CHARGE-ABILITY.md |
| 19 | No OPS-RUNBOOK | Blocks production ops | New doc: docs/lazer-lending/OPS-RUNBOOK.md |
| 20 | No system health dashboard (per-mailbox health is built; system-level isn't) | Operational gap | PLAN.md Phase 1 Task 1.0d |
| 21 | No campaign preview / dry-run flow | UX gap | PLAN.md Phase 1 Task 1.0e |

### Research-validated corrections

| Original audit claim | Correction (per research) | Action |
|----------------------|---------------------------|--------|
| Smartlead is "most reliable cold-mail orchestrator" | 49+ outages/12mo, no SLA, no public status page | Soften framing in BRIEF + PLAN |
| "5–10% Mailforge deplatform probability" | Two-risk-type framing: individual mailbox suspension 20–40%, reseller deplatform <5% | BRIEF + PLAN risk section |
| "$1.67/mailbox Mailforge pricing" | $3/mailbox standard tier; $1.67 is volume discount only | BRIEF D3 + PLAN §Stack |
| "Florida § 501.059 applies to email" | § 501.059 is telephone/SMS only. Correct: Chapter 494 + Rule 69V-40 | COMPLIANCE.md state table |
| "NY GBL § 369-aa governs" | Provision does not appear in current NY codification. Correct: NY Banking Law Article 12-D + 3 NYCRR Part 38.2 | COMPLIANCE.md state table |
| "TCPA applies to email solicitations" | TCPA covers calls/texts only; risk is multi-channel sequences | COMPLIANCE.md §5 |
| "GLBA exempts mortgage broker prospect records from CCPA" | GLBA exemption is data-level. Pre-application prospect records ARE subject to CCPA right-to-delete | COMPLIANCE.md §7 + PLAN.md Phase 1 Task 1.0b |
| "$51,744/violation CAN-SPAM penalty" | $53,088 (Jan 2024 inflation adjustment) | COMPLIANCE.md §2 |
| "2–4 week Mailforge recovery" | 7–10 weeks cold; 24–72 hours with hot standby (which the plan didn't provision) | BRIEF D8 (hot standby required) + Phase 0 Task 0.9 |
| "Build fee $85k–$110k" | At market upper edge but defensible. Recommend $95k floor + 10% contingency | CHARGE-ABILITY.md |
| "$1,800/mo retainer" | Structurally thin under any incident month. Recommend $2,200/mo with 10 hr cap, $150/hr overage | CHARGE-ABILITY.md |

### Net plan impact

- **PLAN.md** v2.1 → v2.5: 21 edits applied (10 pseudocode fixes + 8 new locked decisions D17–D24 + 9 new tasks across Phase 0/1/2 + new Final Validation Checklist + new Schedule Reality section).
- **BRIEF.md**: D3 pricing fix + 3 new decisions (BRIEF.D8/D9/D10) + table refreshes + Instantly disqualification documented.
- **7 new docs** created: PRD-AMENDMENT.md, COMPLIANCE.md, CHARGE-ABILITY.md, VENDOR-CONTRACTS.md, OPS-RUNBOOK.md, WARMUP-CAPABILITY-MAP.md, CONNECT-CRM-AUDIT-DELTA.md.

Plan v2.5 is sufficient for: (a) Lazer to sign PRD-AMENDMENT, (b) IntegrateAPI to send a defensible engagement letter, (c) a fresh implementer to start Phase 0 with no other context.
